(() => {
  ...
  function playQuestion({ replay = false } = {}) {
    ...
    el.copy.textContent = "原版语音正在报题。听到题目后，如果不能忍，就睁眼。";
    el.next.disabled = true;
    state.phase = "audio";
    playFile(q.audio, startObservation, replay ? "正在重播原版题目语音…" : "正在播放原版题目语音…");
  ...
  function openingFinished() {
    state.phase = "openingDone";
    el.phase.textContent = "开场结束";
    el.question.textContent = "准备好就进入第一题";
    el.copy.textContent = "从这里开始，每一题都会由点击操作触发原版语音，手机端也更稳定。";
    el.audioState.textContent = "原版开场语音播放完毕";
    el.next.innerHTML = '进入第一题 <span aria-hidden="true">→</span>';
    el.next.disabled = false;
  }
  ...
  function startRound() {
    ...
    renderProgress();
    el.card.classList.remove("is-observing", "is-discussing");
    el.phase.textContent = "原版开场";
    el.question.textContent = "请双方闭眼";
    el.copy.textContent = "先听纸包鱼老师原版开场语音，听完后点击进入第一题。";
    el.next.disabled = true;
    el.replay.disabled = true;
    el.skip.disabled = true;
    report("tolerance_start", String(state.total), "paperfish_original_v1");
    playFile(OPENING_AUDIO, openingFinished, "正在播放原版开场语音…");
  }
